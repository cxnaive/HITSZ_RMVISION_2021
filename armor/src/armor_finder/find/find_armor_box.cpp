...
// 如果分类器可用，则使用分类器对装甲板候选区进行筛选
    if (classifier) {
        for (auto &armor_box : armor_boxes) {
            cv::Mat proi = src(armor_box.rect);
            //NumDetector::GammaCorrection(proi,roi,1);
            cv::resize(proi, proi, cv::Size(224, 224));
            int c = classifier(proi);
            //cv::imshow("net",proi);
            //waitKey(1);
            if(c == 0) LOG(INFO) <<"inference " << c << "!";
            armor_box.id = c;
        }
// 按照优先级对装甲板进行排序
        ...
        if (config.save_labelled_boxes) {
            for (const auto &one_box : armor_boxes) {
                char filename[100];
                //sprintf(filename, "/home/ubuntu/armor_box_photo/%s_%d.jpg", id2name[one_box.id].data(),time(nullptr) + clock());
                //sprintf(filename, "/home/ubuntu/armor_box_photo/B10_B/%s_%d.jpg", id2name[one_box.id].data(),time(nullptr) + clock());
