...
std::pair<cv::Point3d,cv::Point3d>  ArmorBox::armorSolvePnP() const {
    ...
    //PnP解算
    cv::solvePnP(world_points,getArmorPoints(),config.camConfig.mtx,config.camConfig.dist,Rvec,Tvec,false,cv::SOLVEPNP_ITERATIVE);
    //旋转向量转换为旋转矩阵
    ...
    ans.first = {theta_x * (180 / PI),theta_y * (180 / PI),theta_z * (180 / PI)};
    ans.second = {Tvec.at<double>(0),Tvec.at<double>(1),Tvec.at<double>(2)};
    
    if(config.show_pnp_axies){
        cv::invert(rotM,invrotM);
        P = -invrotM * Tvec;
        std::vector<cv::Point3f> refer_world_points = {cv::Point3f(0,0,0),cv::Point3f(ans.second.z / 10,0,0),cv::Point3f(0,ans.second.z / 10,0),cv::Point3f(0,0,ans.second.z / 10)};
